                      ...
                      int nTries, int timeout, const char *filename)
{
    ...
    if(connectionParameters.role == LlTx){
      ...
      if(llwrite(fd, start_Packet, (start_Pos+strlen(filename))) == -1)   {
      ...
      while(bytes_Remaining > 0){
       ...
       // unsigned char chunk_buffer[chunk_size] = {0};
        ...
        //Envia o data packet
        if(llwrite(fd,data_Packet, dataPacket_length) == -1){
     ...
     if(llwrite(fd, end_Packet, controlPacket_length)){
     ...
     llclose(fd, 1); //vamos fechar o link
    ...
    if(connectionParameters.role == LlRx){
      ...
      while(1){
        ...
        while(((received_length = llread(fd,(incoming_Packet) < 0))&& (transmissions_tries >1))){ // Este loop serve para ler até receber um packet válido
     transmissions_tries --;     
        }
